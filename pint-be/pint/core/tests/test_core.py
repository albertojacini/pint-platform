...
#     monkeypatch.setattr(
#         "pint.core.utils.random_data.get_image", Mock(return_value=image)
#     )
#     for _ in random_data.create_channels():
#         pass
#     for _ in random_data.create_shipping_zones():
...
def test_generate_unique_slug_for_slug_with_max_characters_number(initiative):
    slug = "a" * 256
    result = generate_unique_slug(initiative, slug)
    initiative.slug = result
    with pytest.raises(DataError):
        initiative.save()


def test_generate_unique_slug_non_slugable_value_and_slugable_field(initiative):
    with pytest.raises(Exception):
        generate_unique_slug(initiative)
...
@override_settings(DEBUG=False)
def test_cleardb_passes_with_force_flag_in_debug_off(db):
