...
def get_initiative_export_fields_and_headers_info(
    ...
    all headers.
    Headers contains initiative, variant, attribute and warehouse headers.
    """
    export_fields, file_headers = get_initiative_export_fields_and_headers(export_info)
...
def get_initiative_export_fields_and_headers(
    export_info: Dict[str, list]
    ...
    fields_mapping = dict(
        ChainMap(
            *reversed(
                InitiativeExportFields.HEADERS_TO_FIELDS_MAPPING.values()
            )  # type: ignore
        )
    )
...
#
#     Headers are build from slug and contains information if it's a initiative or variant
#     attribute. Respectively for initiative: "slug-value (initiative attribute)"
...
#
#     initiatives_headers = (
#         attributes.filter(initiative_types__isnull=False)
#         .distinct()
#         .annotate(header=Concat("slug", V(" (initiative attribute)")))
#         .values_list("header", flat=True)
#     )
#
#     variant_headers = (
#         attributes.filter(initiative_variant_types__isnull=False)
#         .distinct()
#         .annotate(header=Concat("slug", V(" (variant attribute)")))
#         .values_list("header", flat=True)
#     )
#
#     return list(initiatives_headers) + list(variant_headers)
#
#
...
#
#     fields = [
#         *InitiativeExportFields.INITIATIVE_CHANNEL_LISTING_FIELDS.keys(),
#         *InitiativeExportFields.VARIANT_CHANNEL_LISTING_FIELDS.keys(),
#     ]
#     channels_headers = []
