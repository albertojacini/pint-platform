...
from . import InitiativeExportFields
...
def get_initiatives_data(
    ...
    """Create data list of initiatives and their variants with fields values.

    It return list with initiative and variant data which can be used as import to
    csv writer and list of attribute and warehouse headers.
    """

    initiatives_with_variants_data = []
    export_variant_id = "variants__id" in export_fields

    initiative_fields = set(
        InitiativeExportFields.HEADERS_TO_FIELDS_MAPPING["fields"].values()
    )
    initiative_export_fields = export_fields & initiative_fields
    if not export_variant_id:
        initiative_export_fields.add("variants__id")

    initiatives_data = (
        queryset.annotate(
            initiative_weight=Case(
        ...
        .values(*initiative_export_fields)
        .distinct("pk", "variants__pk")
    )

    initiatives_relations_data = get_initiatives_relations_data(
    ...
    for initiative_data in initiatives_data:
        pk = initiative_data["id"]
        if export_variant_id:
            variant_pk = initiative_data.get("variants__id")
        else:
            variant_pk = initiative_data.pop("variants__id")

        initiative_relations_data: Dict[str, str] = initiatives_relations_data.get(
            pk, {}
        )
        ...
        initiative_data["id"] = graphene.Node.to_global_id("Initiative", pk)
        if export_variant_id:
            initiative_data["variants__id"] = graphene.Node.to_global_id(
                "InitiativeVariant", variant_pk
            )

        # data = {**initiative_data, **initiative_relations_data, **variant_relations_data}
        data = {**initiative_data, **initiative_relations_data}

        initiatives_with_variants_data.append(data)

    return initiatives_with_variants_data


def get_initiatives_relations_data(
    queryset: "QuerySet",
    export_fields: Set[str],
    attribute_ids: Optional[List[int]],
    channel_ids: Optional[List[int]],
) -> Dict[int, Dict[str, str]]:
    """Get data about initiative relations fields.

    If any many to many fields are in export_fields or some attribute_ids exists then
    dict with initiative relations fields is returned.
    Otherwise it returns empty dict.
    """
    many_to_many_fields = set(
        InitiativeExportFields.HEADERS_TO_FIELDS_MAPPING[
            "initiative_many_to_many"
        ].values()
    )
    relations_fields = export_fields & many_to_many_fields
    if relations_fields or attribute_ids or channel_ids:
        return prepare_initiatives_relations_data(
...
def prepare_initiatives_relations_data(
    queryset: "QuerySet",
    fields: Set[str],
    attribute_ids: Optional[List[int]],
    channel_ids: Optional[List[int]],
) -> Dict[int, Dict[str, str]]:
    """Prepare data about initiatives relation fields for given queryset.

    It return dict where key is a initiative pk, value is a dict with relation fields data.
    """
    # attribute_fields = InitiativeExportFields.INITIATIVE_ATTRIBUTE_FIELDS
    # channel_fields = InitiativeExportFields.INITIATIVE_CHANNEL_LISTING_FIELDS.copy()
    ...
    for data in relations_data.iterator():
        ...
        #     pk, data, attribute_ids, result_data, attribute_fields, "initiative attribute"
        # )
...
#     """
#     many_to_many_fields = set(
#         InitiativeExportFields.HEADERS_TO_FIELDS_MAPPING["variant_many_to_many"].values()
#     )
#     relations_fields = export_fields & many_to_many_fields
...
#     It return dict where key is a initiative pk, value is a dict with relation fields data.
#     """
#     attribute_fields = InitiativeExportFields.VARIANT_ATTRIBUTE_FIELDS
#     warehouse_fields = InitiativeExportFields.WAREHOUSE_FIELDS
#     channel_fields = InitiativeExportFields.VARIANT_CHANNEL_LISTING_FIELDS.copy()
#
#     result_data: Dict[int, dict] = defaultdict(dict)
...
# ) -> Dict[int, dict]:
#     """Add collection info to initiative data.
#
#     This functions adds info about collection to dict with initiative data.
#     If some collection info already exists in data, collection slug is added
#     to set with other values.
#     It returns updated initiative data.
#     """
#
#     if collection:
...
def add_image_uris_to_data(
    ...
    """Add absolute uri of given image path to initiative or variant data.

    This function based on given image path creates absolute uri and adds it to dict
    with variant or initiative data. If some info about images already exists in data,
...
#         reference_initiative=data.pop(attribute_fields["value_reference_initiative"], None),
#     )
#
...
#     result_data: Dict[int, dict],
# ) -> Dict[int, dict]:
#     """Add info about attribute to variant or initiative data.
#
#     This functions adds info about attribute to dict with variant or initiative data.
...
#         attribute_data.reference_page or attribute_data.reference_initiative
#     ):
#         if attribute_data.reference_page:
#             reference_id = attribute_data.reference_page
#         else:
#             reference_id = attribute_data.reference_initiative
...
#
#     This functions adds info about channel to dict with initiative data.
#     It returns updated data.
#     """
