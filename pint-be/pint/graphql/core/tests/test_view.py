...
def test_invalid_syntax_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 400
    assert graphql_log_handler.messages == [
        "pint.graphql.errors.handled[INFO].GraphQLSyntaxError"
    ]
...
def test_permission_denied_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "pint.graphql.errors.handled[INFO].PermissionDenied"
    ]
...
@mock.patch("pint.graphql.product.schema.resolve_collection_by_id")
def test_unexpected_exceptions_are_logged_in_their_own_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "pint.graphql.errors.unhandled[ERROR].NotImplementedError"
    ]
...
@mock.patch("pint.graphql.views.cache.set")
@mock.patch("pint.graphql.views.cache.get")
