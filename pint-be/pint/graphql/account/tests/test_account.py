...
def test_query_customer_user_app(
    ...
    variables = {"id": ID}
    app.permissions.add(
        permission_manage_staff, permission_manage_users
...
def test_query_staff_user(
    ...
    assert len(data["userPermissions"]) == 2
    assert len(data["editableGroups"]) == Group.objects.count()
...
# @patch("pint.plugins.manager.PluginsManager.notify")
def test_customer_register_disabled_email_confirmation(api_client, db):
...
CUSTOMER_CREATE_MUTATION = """
    mutation CreateCustomer(
        ...
        # $channel: String
        $note: String,
        # $billing: AddressInput, $shipping: AddressInput,
...
def test_customer_update_generates_event_when_deactivating_as_app(
    app_api_client, staff_user, customer_user, permission_manage_users
...
@patch("pint.core.tasks.delete_from_storage_task.delay")
@freeze_time("2018-05-31 12:00:01")
...
def test_staff_update_groups_and_permissions(
    ...
    group1.permissions.add(permission_manage_users)
    group2.permissions.add(permission_manage_initiatives)
    ...
    staff_api_client.user.user_permissions.add(
        permission_manage_users, permission_manage_initiatives
...
def test_staff_update_duplicated_input_items(
    ...
    staff_user = User.objects.create(email="staffuser@example.com", is_staff=True)
    staff_api_client.user.user_permissions.add(
        permission_manage_initiatives, permission_manage_users
    )
    id = graphene.Node.to_global_id("User", staff_user.id)
    variables = {
        "id": id,
...
def test_unauthenticated_query_group_federation(api_client, db):
    group = Group.objects.create(name="empty group")
