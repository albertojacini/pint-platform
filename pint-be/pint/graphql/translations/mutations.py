...
class InitiativeTranslate(BaseTranslateMutation):
    class Arguments:
        id = graphene.ID(
            required=True,
            description="Initiative ID or InitiativeTranslatableContent ID.",
        )
        language_code = graphene.Argument(
            LanguageCodeEnum, required=True, description="Translation language code."
        )
    ...
    class Meta:
        description = "Creates/updates translations for a initiative."
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        initiative = cls.get_node_or_error(info, node_id, only_type=Initiative)
        cls.validate_input(data["input"])

        translation, created = initiative.translations.update_or_create(
            language_code=data["language_code"], defaults=data["input"]
        )
