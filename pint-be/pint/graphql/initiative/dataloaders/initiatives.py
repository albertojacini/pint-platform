...
class InitiativeByIdLoader(DataLoader):
    context_key = "initiative_by_id"

    def batch_load(self, keys):
        initiatives = Initiative.objects.using(self.database_connection_name).in_bulk(keys)
        return [initiatives.get(initiative_id) for initiative_id in keys]
...
class MediaByInitiativeIdLoader(DataLoader):
    context_key = "media_by_initiative"

    def batch_load(self, keys):
        media = InitiativeMedia.objects.using(self.database_connection_name).filter(
            initiative_id__in=keys,
        )
        media_map = defaultdict(list)
        for media_obj in media.iterator():
            media_map[media_obj.initiative_id].append(media_obj)
        return [media_map[initiative_id] for initiative_id in keys]
...
class ImagesByInitiativeIdLoader(DataLoader):
    context_key = "images_by_initiative"

    def batch_load(self, keys):
        images = InitiativeMedia.objects.using(self.database_connection_name).filter(
            initiative_id__in=keys,
...
class InitiativeMediaByIdLoader(DataLoader):
    context_key = "initiative_media_by_id"

    def batch_load(self, keys):
        initiative_media = InitiativeMedia.objects.using(
            self.database_connection_name
        ).in_bulk(keys)
        return [initiative_media.get(initiative_media_id) for initiative_media_id in keys]


class InitiativeImageByIdLoader(DataLoader):
    context_key = "initiative_image_by_id"

    def batch_load(self, keys):
        images = (
            InitiativeMedia.objects.using(self.database_connection_name)
            .filter(type=InitiativeMediaTypes.IMAGE)
            .in_bulk(keys)
        )
        return [images.get(initiative_image_id) for initiative_image_id in keys]


class InitiativeImageByInitiativeIdLoader(DataLoader):
    context_key = "initiative_image_by_initiative_id"

    def batch_load(self, keys):
        medias = InitiativeMedia.objects.using(self.database_connection_name).filter(
            type=InitiativeMediaTypes.IMAGE,
            initiative_id__in=keys,
        )
        initiative_id_medias_map = defaultdict(list)
        for media in medias.iterator():
            initiative_id_medias_map[media.initiative_id].append(media)
        return [initiative_id_medias_map.get(initiative_id, []) for initiative_id in keys]
