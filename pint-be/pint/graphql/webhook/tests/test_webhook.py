...
def test_webhook_create_by_app(app_api_client, permission_manage_initiatives):
    ...
    variables = {
        "input": {
            ...
            "asyncEvents": [
                WebhookEventTypeAsyncEnum.INITIATIVE_CREATED.name,
                WebhookEventTypeAsyncEnum.INITIATIVE_CREATED.name,
            ],
        }
    }
    ...
    assert len(events) == 1
    assert events[0].event_type == WebhookEventTypeAsyncEnum.INITIATIVE_CREATED.value


def test_webhook_create_inactive_app(app_api_client, app, permission_manage_initiatives):
    app.is_active = False
    app.save()
    query = WEBHOOK_CREATE
    variables = {
        "input": {
            "targetUrl": "https://www.example.com",
            "asyncEvents": [WebhookEventTypeAsyncEnum.INITIATIVE_CREATED.name],
            "name": "",
        }
    }
...
def test_webhook_create_by_staff_invalid_query(
    app_api_client, permission_manage_initiatives
):
    query = WEBHOOK_CREATE
    variables = {
        "input": {
...
#     staff_api_client, subscription_initiative_created_webhook, permission_manage_apps
# ):
#     query = QUERY_WEBHOOK
#     webhook = subscription_initiative_created_webhook
#     webhook_id = graphene.Node.to_global_id("Webhook", webhook.pk)
#     variables = {"id": webhook_id}
#     staff_api_client.user.user_permissions.add(permission_manage_apps)
#     response = staff_api_client.post_graphql(query, variables=variables)
...
def test_query_webhook_by_app_without_permission(app_api_client):
    ...
    webhook.events.create(event_type="initiative_created")
...
def test_query_webhook_sync_and_async_events(
    ...
    webhook.events.create(event_type=WebhookEventTypeAsyncEnum.INITIATIVE_CREATED.value)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # (WebhookSampleEventTypeEnum.INITIATIVE_CANCELLED, True),
        # (WebhookSampleEventTypeEnum.INITIATIVE_FULFILLED, True),
