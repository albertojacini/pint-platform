...
def _prep_initiative_search_vector_index(initiatives):
    prefetch_related_objects(initiatives, *PRODUCT_FIELDS_TO_PREFETCH)
    for initiative in initiatives:
        initiative.search_vector = FlatConcatSearchVector(
            *prepare_initiative_search_vector_value(initiative, already_prefetched=True)
        )
        initiative.search_index_dirty = False

    Initiative.objects.bulk_update(
        initiatives, ["search_vector", "updated_at", "search_index_dirty"]
    )


def update_initiatives_search_vector(initiatives: "QuerySet", use_batches=True):
    if use_batches:
        last_id = 0
        while True:
            initiatives_batch = list(initiatives.filter(id__gt=last_id)[:PRODUCTS_BATCH_SIZE])
            if not initiatives_batch:
                break
            last_id = initiatives_batch[-1].id
            _prep_initiative_search_vector_index(initiatives_batch)
    else:
        _prep_initiative_search_vector_index(initiatives)


def update_initiative_search_vector(initiative: "Initiative"):
    initiative.search_vector = FlatConcatSearchVector(
        *prepare_initiative_search_vector_value(initiative)
    )
    initiative.save(update_fields=["search_vector", "updated_at"])


def prepare_initiative_search_vector_value(
    initiative: "Initiative", *, already_prefetched=False
    ...
    search_vectors = [
        ...
        #     initiative.attributes.all()[: settings.PRODUCT_MAX_INDEXED_ATTRIBUTES]
        # ),
        # *generate_variants_search_vector_value(initiative),
    ]
...
# ) -> List[NoValidationSearchVector]:
#     variants = list(initiative.variants.all()[: settings.PRODUCT_MAX_INDEXED_VARIANTS])
#
#     search_vectors = [
#         NoValidationSearchVector(
...
def search_initiatives(qs, value):
    if value:
