...
import { InitiativeChannelListingAddInput } from "@saleor/graphql";
...
export function createInitiativeTypeSelectHandler(
  setInitiativeType: (initiativeTypeId: string) => void,
  triggerChange: () => void,
): FormChange {
  return (event: React.ChangeEvent<any>) => {
    const id = event.target.value;
    setInitiativeType(id);
    triggerChange();
  };
}
