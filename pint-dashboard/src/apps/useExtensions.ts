...
export const extensionMountPoints = {
  INITIATIVE_LIST: [
    AppExtensionMountEnum.INITIATIVE_OVERVIEW_CREATE,
    AppExtensionMountEnum.INITIATIVE_OVERVIEW_MORE_ACTIONS,
  ],
  ORDER_LIST: [
  ...
  INITIATIVE_DETAILS: [AppExtensionMountEnum.INITIATIVE_DETAILS_MORE_ACTIONS],
  NAVIGATION_SIDEBAR: [
...
export const mapToMenuItemsForInitiativeDetails = (
  extensions: ExtensionWithParams[],
  initiativeId: string,
) =>
  extensions.map(extension =>
    mapToMenuItem({ ...extension, open: () => extension.open({ initiativeId }) }),
  );
